...
BASE_SEARCH_CONFIG=/mnt/common/OPEN_DATA/FLEXTYPER_INDEXES/INIFILES/${SAMPLE}_HLA.ini
for query in $(ls /mnt/common/OPEN_DATA/FLEXTYPER_INDEXES/HLA/*HLA_MajorAlleles_query.tsv )
do
	echo $query
	...
	QUERY_SEARCH_CONFIG=$INDEX_DIR/${SAMPLE}_HLA_MajorAlleles_${queryGene}.ini
	sed -e "s/hla_seqs_query.tsv/${queryGene}_HLA_MajorAlleles_query.tsv/g" $BASE_SEARCH_CONFIG > $QUERY_SEARCH_CONFIG
	
	$FLEXTYPER search \
		-v \
		-c $QUERY_SEARCH_CONFIG \
		-k 31 -s 1 -m 150 \
		${SAMPLE}_${queryGene}_HLA_MajorAlleles_FTSearch_UniqReads
